...
class _SignupSubScreenState extends State<SignupSubScreen> {
  ...
  void _formatPhoneNumber() {
    ...
    // 최대 11자리까지만 허용
    final limitedText = cleanedText.length > 11 ? cleanedText.substring(0, 11) : cleanedText;
    ...
    if (limitedText.length <= 3) {
      ...
      cursorOffset = limitedText.length;
    } else if (limitedText.length <= 7) {
      formattedText = '${limitedText.substring(0, 3)}-${limitedText.substring(3)}';
      cursorOffset = formattedText.length;
    } else {
      // 11자리까지 안전하게 처리
      formattedText = '${limitedText.substring(0, 3)}-${limitedText.substring(3, 7)}-${limitedText.substring(7)}';
      cursorOffset = formattedText.length;
    }
    
    // 리스너 무한 루프 방지를 위해 값이 실제로 변경된 경우에만 업데이트
          ...
          child: Column(
            ...
            children: [
              ...
              _buildInputField(
                ...
                onChanged: (value) => setState(() => _validateTeacher(value)),
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF00FF9D), // 이미지와 동일한 형광초록
